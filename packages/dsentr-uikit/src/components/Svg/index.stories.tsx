...
const context: __WebpackModuleApi.RequireContext = require.context('./Icons', true, /.tsx$/)

const components = context.keys().reduce<Record<string, object>>((accum: object, path: string) => {
  const file: string = path.substring(2).replace('.tsx', '')
  return {
    ...accum,
    [file]: context(path),
    default: context(path).default
  }
}, {})
  ...
  return (
    <Flex justifyContent='start' alignItems='center' flexWrap='wrap'>
      {Object.keys(components).map((file) => {
        const Icon = (components[file] as __WebpackModuleApi.RequireContext)(file).default
        return (
          <Flex
            key={file}
