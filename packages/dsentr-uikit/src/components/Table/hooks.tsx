...
const sortByColumn = <T extends DataType>(
  data: Array<RowType<T>>,
  sortColumn: string,
  columns: Array<ColumnStateType<T>>
): Array<RowType<T>> => {
  let isAscending = null
  let sortedRows: Array<RowType<T>> = [...data]
  ...
  columns.forEach((column) => {
    ...
    if (sortColumn === column.name) {
      ...
      if (column.sort != null) {
        sortedRows = isAscending != null && isAscending ? data.sort(column.sort) : data.sort(column.sort).reverse()
...
const getColumnsByName = <T extends DataType>(columns: Array<ColumnType<T>>): ColumnByNamesType<T> => {
...
const createReducer =
  <T extends DataType>() =>
    (state: TableState<T>, action: TableAction<T>): TableState<T> => {
      ...
      let prevPage = 0
      let isAscending = null
      let sortedRows: Array<RowType<T>> = []
      let columnCopy = []
      let filteredRows = []
      ...
      switch (action.type) {
        case 'SET_ROWS':
          ...
          if (state.sortColumn != null && state.sortColumn !== '') {
        ...
        case 'SELECT_ROW':
          ...
          stateCopy.rows = stateCopy.rows.map((row) => {
            ...
            if (newRow.id === action.rowId) {
              newRow.selected = newRow.selected != null && !newRow.selected
            }
            return newRow
          })
          ...
          stateCopy.rows = stateCopy.originalRows.filter((row) => {
            return (
              row.cells.filter((cell) => {
                if (Array(cell.value).includes(action.searchString)) {
                  return true
                }
                return false
              }).length > 0
...
const sortDataInOrder = <T extends DataType>(data: T[], columns: Array<ColumnType<T>>): T[] => {
...
export const makeRender = <T extends DataType>(
  ...
  return render != null ? () => render({ row, value }) : () => value
}

const makeHeaderRender = (label: string, render?: HeaderRenderType): React.ReactNode => {
  return render != null ? () => render({ label }) : () => label
}
...
export const useTable = <T extends DataType>(
  ...
  const columnsWithSorting: Array<ColumnStateType<T>> = useMemo(
    () =>
      columns.map((column) => {
        return {
          ...column,
          label: column.label != null ? column.label : column.name,
          hidden: column.hidden != null && column.hidden ? column.hidden : false,
          sort: column.sort,
          sorted: {
            on: false,
            asc: false
  ...
  const tableData: Array<RowType<T>> = useMemo(() => {
    ...
    const newData = sortedData.map((row, idx) => {
      return {
        ...
        cells: Object.entries(row)
          ...
          .filter((cell) => cell.hidden != null && !cell.hidden)
      }
    })
    return newData
  ...
  const [state, dispatch] = useReducer(reducer, {
    ...
    filterOn: !(options?.filter == null),
    sortColumn: options?.sortColumn,
    paginationEnabled: options?.pagination != null && options?.pagination,
    pagination: {
      page: 1,
      perPage: 10,
      canNext: true,
  ...
  const headers: any = useMemo(() => {
    return [
      ...state.columns.map((column: ColumnStateType<T>) => {
        const label = column.label != null ? column.label : column.name
        return {
          ...column,
  ...
  return {
    headers: headers.filter((column: ColumnStateType<T>) => column.hidden != null && !column.hidden),
    rows: state.rows,
