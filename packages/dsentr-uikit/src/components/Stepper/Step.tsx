...
const getStepNumberFontColor = ({ theme, status }: StatusProps): string => {
  if (status === 'past') {
...
const Connector = styled.div<StatusProps>`
  ...
  top: 50%;
  left: calc(50% - 2px);
  background-color: ${({ theme, status }) => theme.colors[status === 'past' ? 'primary' : 'textDisabled']};
`
...
export const StepNumber = styled.div<StatusProps>`
  ...
  border: 2px solid ${({ theme, status }) => (status === 'past' ? theme.colors.primary : 'transparent')};
