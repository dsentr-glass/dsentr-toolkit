...
declare global {
  ...
  namespace jest {
    interface Matchers<R> {
      toBeDeclaredOnce: (type: string, parameter: string, chainId: number) => R
      toBeValid: (validationErrors: any) => R
    }
  }
}
...
expect.extend({
  ...
  toBeValid (received: boolean, validationErrors) {
    if (received) {
      return {
        message: () => '',
        pass: true
...
describe.each([['dsentr-default'], ['dsentr-extended'], ['dsentr-top-100']])('buildList %s', (listName) => {
  ...
  it('contains no duplicate addresses', () => {
    ...
    for (const token of defaultTokenList.tokens) {
      const key: string = `${token.chainId}-${token.address.toLowerCase()}`
      expect(map[key]).toBeDeclaredOnce('address', token.address.toLowerCase(), token.chainId)
      map[key] = true
    }
  })

  // ORIGINAL FROM PANCAKE SWAP:
  //
