...
This repo contains main DSentr token list and tools to validate it.
...
- Add `checksum:newlistname`, `generate:newlistname`, `makelist:newlistname` command to `package.json` analogous to DSentr default and extended list scripts.
- Modify `checksum.ts`, `buildList.ts` and `default.test.ts` to handle new list

## How to add new tokens to DSentr (extended) token list
...
- Add new tokens to `src/tokens/dsentr-extended.json` file
...
```shell script
# Fetch the Top100 Tokens on DSentr, and update list.
$ yarn fetch:pcs-top-100

# Build token list (dsentr-top-100.json)
...
Netlify simply takes the json files under `lists` directory and hosts them on `tokens.dsentr.com/list-name.json`
