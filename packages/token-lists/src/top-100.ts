...
/**
 * Fetch Top100 Tokens traded on DSentr, ordered by trading volume,
...
const getTokens = async (): Promise<any> => {
  try {
    ...
    console.error(`Error when fetching Top100 Tokens by volume for the past 30 days, error: ${error.message as string}`)
  }
}
...
const main = async (): Promise<void> => {
  ...
  const sanitizedTokens = tokens.reduce((list: any, item: BitqueryEntity) => {
  ...
  const tokenListPath = `${path.resolve()}/src/tokens/dsentr-top-100.json`
