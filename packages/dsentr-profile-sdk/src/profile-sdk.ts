...
interface SdkConstructorArguments {
  web3?: Web3
  chainId?: number
}

class DSentrProfileSdk {
  web3 = web3NoAccount
  ...
  constructor (args?: SdkConstructorArguments) {
    if ((args?.web3) != null) this.web3 = args.web3
    if (args?.chainId != null) this.chainId = args.chainId
   ...
   * API repo - https://github.com/dsentr-glass/dsentr-profile-api
   */
   ...
   * Contract repo - https://github.com/dsentr-glass/dsentr-contracts/tree/master/projects/profile-nft-gamification
   */
  getTeam = async (teamId: number | undefined): Promise<Team> => {
    const fallback = {
      id: -1,
      name: '',
      description: '',
    ...
    try {
      ...
      const staticTeamInfo: Team = (teamsList.find(staticTeam => staticTeam.id === teamId)) || fallback
   ...
   * from DSentrProfile contract.
   * NFT's shardId is retrieved from GlassShards contract and mapped to static NFT data stored in constant.
   * Contracts repo - https://github.com/dsentr-glass/dsentr-contracts/tree/master/projects/profile-nft-gamification
   */
  getProfile = async (address: string): Promise<GetProfileResponse> => {
    try {
      ...
      if (!hasRegistered) {
        return { hasRegistered, profile: undefined }
      }
      ...
      let nft: Nft | null | undefined
      if (isActive != null && isActive && nftAddress != null && tokenId != null) {
        nft = await getNftByTokenId(nftAddress, tokenId, this.web3, this.chainId)
        const avatar = (nft != null) ? `https://dsentr.com/images/nfts/${nft.images.sm}` : undefined
      ...
      if (userId == null || points == null || teamId == null || tokenId == null || nftAddress == null || isActive == null || nft == null) {
        throw new Error('Some items were undefined')
      }
