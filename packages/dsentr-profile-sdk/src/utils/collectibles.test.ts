...
const GLASS_NFT_ADDRESS = '0xDf7952B35f24aCF7fC0487D01c8d5690a60DBa07'
const MIRROR_NFT_ADDRESS = '0xa251b5EAa9E67F2Bc8b33F33e20E91552Bf85566'
...
describe('collectibles', () => {
  it('getIdentifierKeyFromAddress returns proper identifier key', () => {
    const glassIdentifierKey = getIdentifierKeyFromAddress(GLASS_NFT_ADDRESS, MAINNET_CHAIN_ID)
    const mirrorIdentifierKey = getIdentifierKeyFromAddress(MIRROR_NFT_ADDRESS, MAINNET_CHAIN_ID)
    expect(glassIdentifierKey).toBe('image')
    expect(mirrorIdentifierKey).toBe('otherIdentifier')
  })
  ...
  it('getTokenUriData returns proper response', async () => {
    const uriData = await getTokenUriData(GLASS_NFT_ADDRESS, MOCK_TOKEN_ID, web3NoAccount)
    expect(uriData).toEqual({
      name: 'Prism',
      description: 'Glass works colors',
      image: 'ipfs://QmYD9AtzyQPjSa9jfZcZq88gSaRssdhGmKqQifUDjGFfXm/prism.png',
      attributes: {
        shardId: '5'
      }
    })
  ...
  it('getTokenUriData returns null if request failed', async () => {
    server.use(
      rest.get(
        ...
        async (req, res, ctx) => {
          return await res(ctx.status(500), ctx.json({ message: '500 Internal Server Error' }))
        }
      )
    )
    const uriData = await getTokenUriData(GLASS_NFT_ADDRESS, MOCK_TOKEN_ID, web3NoAccount)
  ...
  it('getNftByTokenId returns proper nft', async () => {
    ...
    const nft = await getNftByTokenId(GLASS_NFT_ADDRESS, MOCK_TOKEN_ID, web3NoAccount, MAINNET_CHAIN_ID)
  ...
  it('getNftByTokenId returns null if uriData does not contain indentifierKey', async () => {
    ...
    const nft = await getNftByTokenId(MIRROR_NFT_ADDRESS, MOCK_TOKEN_ID, web3NoAccount, MAINNET_CHAIN_ID)
    expect(nft).toBe(null)
  })
})
