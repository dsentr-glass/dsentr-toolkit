    ...
    methods: {
      hasRegistered: jest.fn((callAddress: string) => {
        if (callAddress === existingAddress1) {
          return { call: jest.fn(async () => await Promise.resolve(true)) }
        }
        return { call: jest.fn(async () => await Promise.resolve(false)) }
      }),
      getUserProfile: jest.fn((callAddress: string) => {
        ...
        return { call: jest.fn(async () => await Promise.resolve(null)) }
      }),
  ...
  return {
    methods: {
      tokenURI: jest.fn((tokenId: string) => {
        return {
          call: jest.fn(async () => await Promise.resolve('ipfs://QmYsTqbmGA3H5cgouCkh8tswJAQE1AsEko9uBZX9jZ3oTC/sleepy.json'))
        }
      })
    }
  }
